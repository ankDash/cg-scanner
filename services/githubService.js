...
const { graphqldata } = require('../common/graphqldata');
const { Octokit } = require("@octokit/rest");
const CONSTANTS = require('../common/constants');
const util = require('../utility/FileValidation');
...
class GithubService {
  ...
  constructor() {
    ...
    this.graphqlClient = graphqldata();
  }
  ...
  `;
    ...
    let response = [];
      ...
      response.push({
        ...
        "Files at Root Level":fileCount,  
        "Folders at Root Level":folderCount,
        "YML File":haveYmlFile,
        "YML File Content":text,
  ...
  async haveYmlFiles (filesArr,repoOwnerName,repoName) {    
    ...
    for(var i=0;i<filesArr.entries.length;i++){ 
      ...
      if(splitArr[1]){        
        if(await util.fileValidation(filesArr.entries[i].name,CONSTANTS.YML_EXTENSION)) {      
